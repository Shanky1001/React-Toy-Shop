export const cartReducer = (state, action) => {
     switch (action.type) {
          ...
          case "increaseQTY":
               return { ...state, cart: state.cart.filter((c) => (c.id === action.payload.id) ? c.qty = (action.payload.qty) : c.qty), total: action.payload.total }

          case "decreaseQTY":
               return { ...state, cart: state.cart.filter((c) => (c.id === action.payload.id) ? (c.qty === 1 ? c.qty = 1 : c.qty = action.payload.qty) : c.qty), total: action.payload.total }
