    ...
    if (formData.email === "" || formData.pass === "") {
      ...
      data.forEach((val) => {
        if (val.email === formData.email && val.pass === formData.pass) {
          ...
          setOpenSnack({ open: true, html: `Welcome ${val.name}`, severity: "success", time: "1000" })
    ...
    <div className={style.login}>
      <div className={style.signContainer}>
        ...
        <form id='registerForm' className={style.formContainer}>
          <div>
            <h1>Email-Id</h1>
            <input type='email' placeholder='email@abc.com' required autoFocus value={formData.email} name='email' onChange={log} />
            <h1> Password </h1>
            <input type='password' placeholder='password' required name='pass' value={formData.pass} onChange={log} />
            <p id="notice"></p>
