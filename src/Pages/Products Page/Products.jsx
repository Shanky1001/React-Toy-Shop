...
const Products = () => {
  ...
  const confirmed = () => {
    ...
    if (search !== '') {
      sortedProducts = sortedProducts.filter((p) => p.title.toLowerCase().trim().includes(search.toLowerCase().trim()));
    }
    if (byRating !== '')
      sortedProducts = sortedProducts.filter((p) => p.rating === Number(byRating))

    if (byPrice !== '') {
      switch (Number(byPrice)) {
        case 1:
          return sortedProducts = sortedProducts.filter((p) => p.price > 1 && p.price < 1000)
        case 2:
          return sortedProducts = sortedProducts.filter((p) => p.price >= 1001 && p.price <= 2000)
        case 3:
          return sortedProducts = sortedProducts.filter((p) => p.price >= 2001 && p.price <= 3000)
    ...
    if (byAge !== '') {
      switch (Number(byAge)) {
        case 1:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 1 && p.maxAge <= 1)
        case 3:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 18 && p.maxAge < 36)
        case 5:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 3 && p.maxAge < 5)
        case 7:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 5 && p.maxAge < 7)
        case 9:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 7 && p.maxAge < 9)
        case 12:
          return sortedProducts = sortedProducts.filter((p) => p.minAge >= 9 && p.maxAge < 12)
        case 13:
          ...
          <DialogContent>
            <Box component="form" sx={{ display: 'flex', flexWrap: 'wrap' }}>
              <FormControl sx={{ m: 1, minWidth: 120 }}>
                ...
                }} name='age' input={<OutlinedInput label="Category" />}  >
              ...
              <FormControl sx={{ m: 1, minWidth: 120 }}>
                ...
                <Select labelId="demo-dialog-select-label" name='rating' onChange={(e) => {
                  productDispatch({
                    type: 'byRating',
                    payload: e.target.value
                  })
              ...
              <FormControl sx={{ m: 1, minWidth: 120 }}>
                ...
                <Select labelId="demo-dialog-select-label" name='price' onChange={(e) => {
                  productDispatch({
                    type: 'byPrice',
                    payload: e.target.value
                  })
